...
fn to_style(s: TaffyUGUIStyle) -> Style {
    Style {
        ...
        max_size: Size { width: dim(s.max_width), height: dim(s.max_height) },
        flex_basis: dim(s.flex_basis),
        flex_grow: s.flex_grow.max(0.0),
        flex_shrink: s.flex_shrink.max(0.0),
        gap: Size { width: LengthPercentage::length(s.gap_x.max(0.0)), height: LengthPercentage::length(s.gap_y.max(0.0)) },
        padding: Rect {
        ...
        aspect_ratio: if s.aspect_ratio > 0.0 { Some(s.aspect_ratio) } else { None },
        ..Default::default()
    }
}
...
#[no_mangle]
pub extern "C" fn taffy_ugui_api_version() -> u32 { 1 }

#[no_mangle]
pub extern "C" fn taffy_ugui_create_context() -> *mut c_void {
    let ctx = Context { tree: TaffyTree::new(), nodes: HashMap::new(), next_id: 1 };
    Box::into_raw(Box::new(ctx)) as *mut c_void
}

#[no_mangle]
pub unsafe extern "C" fn taffy_ugui_destroy_context(ptr: *mut c_void) {
    if !ptr.is_null() { drop(Box::from_raw(ptr as *mut Context)); }
}

#[no_mangle]
pub unsafe extern "C" fn taffy_ugui_create_node(ptr: *mut c_void, style: TaffyUGUIStyle, out_id: *mut u64) -> i32 {
    let Some(ctx) = context(ptr) else { return ERR_NULL; };
    let Some(out_id) = out_id.as_mut() else { return ERR_NULL; };
...
pub unsafe extern "C" fn taffy_ugui_remove_node(ptr: *mut c_void, id: u64) -> i32 {
    let Some(ctx) = context(ptr) else { return ERR_NULL; };
    let Some(node) = ctx.nodes.remove(&id) else { return ERR_NODE; };
    ctx.tree.remove(node).map(|_| OK).unwrap_or(ERR_TAFFY)
}

#[no_mangle]
pub unsafe extern "C" fn taffy_ugui_set_style(ptr: *mut c_void, id: u64, style: TaffyUGUIStyle) -> i32 {
    let Some(ctx) = context(ptr) else { return ERR_NULL; };
    let Some(&node) = ctx.nodes.get(&id) else { return ERR_NODE; };
    ctx.tree.set_style(node, to_style(style)).map(|_| OK).unwrap_or(ERR_TAFFY)
}

#[no_mangle]
pub unsafe extern "C" fn taffy_ugui_set_children(ptr: *mut c_void, id: u64, child_ids: *const u64, count: usize) -> i32 {
    let Some(ctx) = context(ptr) else { return ERR_NULL; };
    let Some(&node) = ctx.nodes.get(&id) else { return ERR_NODE; };
    if count > 0 && child_ids.is_null() { return ERR_NULL; }
    let ids = if count == 0 { &[][..] } else { std::slice::from_raw_parts(child_ids, count) };
    let mut children = Vec::with_capacity(count);
    for child_id in ids {
        let Some(&child) = ctx.nodes.get(child_id) else { return ERR_NODE; };
        children.push(child);
    }
    ctx.tree.set_children(node, &children).map(|_| OK).unwrap_or(ERR_TAFFY)
}
...
#[no_mangle]
pub unsafe extern "C" fn taffy_ugui_compute_layout(ptr: *mut c_void, root_id: u64, width: f32, height: f32) -> i32 {
    let Some(ctx) = context(ptr) else { return ERR_NULL; };
    let Some(&root) = ctx.nodes.get(&root_id) else { return ERR_NODE; };
    let available = Size {
        width: if width.is_finite() { AvailableSpace::Definite(width.max(0.0)) } else { AvailableSpace::MaxContent },
        height: if height.is_finite() { AvailableSpace::Definite(height.max(0.0)) } else { AvailableSpace::MaxContent },
    };
    ctx.tree.compute_layout(root, available).map(|_| OK).unwrap_or(ERR_TAFFY)
}

#[no_mangle]
pub unsafe extern "C" fn taffy_ugui_get_layout(ptr: *mut c_void, id: u64, out_layout: *mut TaffyUGUILayout) -> i32 {
    let Some(ctx) = context(ptr) else { return ERR_NULL; };
    let Some(&node) = ctx.nodes.get(&id) else { return ERR_NODE; };
    let Some(out_layout) = out_layout.as_mut() else { return ERR_NULL; };
