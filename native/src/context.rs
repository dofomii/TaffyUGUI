...
use taffy::{
    ...
    compute_hidden_layout, compute_leaf_layout, compute_root_layout, round_layout, BlockContext, Cache,
    CacheTree, LayoutBlockContainer, LayoutFlexboxContainer, LayoutGridContainer, LayoutPartialTree, RoundTree,
...
impl NativeTree {
    ...
    fn remove_node(&mut self, node: NodeId) -> Result<(), NativeError> {
        ...
        let state = self.nodes.get(index).and_then(Option::as_ref).ok_or(NativeError::NodeNotFound)?;
    ...
    fn node(&self, node: NodeId) -> Result<&NodeState, NativeError> {
        self.nodes.get(usize::from(node)).and_then(Option::as_ref).ok_or(NativeError::NodeNotFound)
    }

    fn node_mut(&mut self, node: NodeId) -> Result<&mut NodeState, NativeError> {
        self.nodes.get_mut(usize::from(node)).and_then(Option::as_mut).ok_or(NativeError::NodeNotFound)
    }

    fn set_style(&mut self, node: NodeId, mut style: Style) -> Result<(), NativeError> {
        if self.node(node)?.measurement.as_ref().is_some_and(|record| record.is_replaced) {
    ...
    fn set_measurement(&mut self, node: NodeId, measurement: Option<MeasurementRecord>) -> Result<(), NativeError> {
        if let Some(record) = &measurement {
            record.validate()?;
        }
        let state = self.node_mut(node)?;
        state.style.item_is_replaced = measurement.as_ref().is_some_and(|record| record.is_replaced);
    ...
    fn set_children(&mut self, parent: NodeId, children: &[NodeId]) -> Result<(), NativeError> {
        ...
        for (index, child) in children.iter().enumerate() {
            self.node(*child)?;
            if *child == parent || children[..index].contains(child) || self.would_create_cycle(parent, *child)? {
        ...
        for child in children {
            if let Some(old_parent) = self.node(*child)?.parent {
                if old_parent != parent {
                    self.node_mut(old_parent)?.children.retain(|candidate| candidate != child);
...
impl TraversePartialTree for NativeTree {
    ...
    fn child_ids(&self, parent_node_id: NodeId) -> Self::ChildIter<'_> {
        ChildIter(self.node(parent_node_id).expect("live parent").children.iter())
    }

    fn child_count(&self, parent_node_id: NodeId) -> usize {
        self.node(parent_node_id).expect("live parent").children.len()
    }
...
impl CacheTree for NativeTree {
    ...
    fn cache_store(&mut self, node_id: NodeId, input: &LayoutInput, output: LayoutOutput) {
        self.node_mut(node_id).expect("live node").cache.store(input, output);
    }
...
impl LayoutGridContainer for NativeTree {
    ...
    fn set_detailed_grid_info(&mut self, node_id: NodeId, detailed_grid_info: DetailedGridInfo) {
        self.node_mut(node_id).expect("live node").detailed_layout_info =
...
impl Context {
    ...
    pub(crate) fn set_style(&mut self, handle: NodeHandle, style: Style) -> Result<(), NativeError> {
        let node = self.nodes.resolve(handle)?;
    ...
    pub(crate) fn apply_grid_template(&mut self, handle: NodeHandle, resource: &GridTemplateResource) -> Result<(), NativeError> {
        let node = self.nodes.resolve(handle)?;
    ...
    pub(crate) fn set_styles_bulk(&mut self, updates: &[(NodeHandle, Style)]) -> Result<(), NativeError> {
    ...
    pub(crate) fn set_children(&mut self, handle: NodeHandle, child_handles: &[NodeHandle]) -> Result<(), NativeError> {
        let node = self.nodes.resolve(handle)?;
    ...
    pub(crate) fn set_children_bulk(&mut self, updates: &[(NodeHandle, Vec<NodeHandle>)]) -> Result<(), NativeError> {
        let resolved = updates
            .iter()
            .map(|(parent, children)| {
                Ok((
                    self.nodes.resolve(*parent)?,
                    children.iter().map(|child| self.nodes.resolve(*child)).collect::<Result<Vec<_>, _>>()?,
                ))
            })
            .collect::<Result<Vec<_>, NativeError>>()?;
    ...
    pub(crate) fn calc_dimension(&self, handle: ResourceHandle) -> Result<Dimension, NativeError> { self.tree.calc.dimension(handle) }
    pub(crate) fn calc_length_percentage(&self, handle: ResourceHandle) -> Result<LengthPercentage, NativeError> { self.tree.calc.length_percentage(handle) }
    pub(crate) fn calc_length_percentage_auto(&self, handle: ResourceHandle) -> Result<LengthPercentageAuto, NativeError> { self.tree.calc.length_percentage_auto(handle) }
    pub(crate) fn calc_ptr(&self, handle: ResourceHandle) -> Result<*const (), NativeError> { self.tree.calc.token_ptr_for_taffy(handle) }

    pub(crate) fn compute_layout(&mut self, root_handle: NodeHandle, width: f32, height: f32) -> Result<(), NativeError> {
        let root = self.nodes.resolve(root_handle)?;
        let available = Size { width: available_space(width), height: available_space(height) };
        let key = ComputeKey { root, mutation_generation: self.mutation_generation, available };
    ...
    pub(crate) fn layouts_bulk(&self, handles: &[NodeHandle]) -> Result<Vec<LayoutResult>, NativeError> {
        handles
            .iter()
            .map(|handle| {
    ...
    pub(crate) fn detailed_layout_info(&self, handle: NodeHandle) -> Result<&DetailedLayoutInfo, NativeError> {
...
impl NodeRegistry {
    fn insert(&mut self, node: NodeId) -> Result<NodeHandle, NativeError> {
        ...
        if let Some(index) = self.free.pop() {
            let slot = self.slots.get_mut(index as usize).ok_or(NativeError::NodeNotFound)?;
        ...
        self.slots.push(NodeSlot { generation, node: Some(node) });
    ...
    fn resolve(&self, handle: NodeHandle) -> Result<NodeId, NativeError> {
        ...
        let slot = self.slots.get(index as usize).ok_or(NativeError::NodeNotFound)?;
        if slot.generation != generation { return Err(NativeError::NodeNotFound); }
    ...
    fn remove(&mut self, handle: NodeHandle) -> Result<(), NativeError> {
        let (index, generation) = handle.parts().ok_or(NativeError::NodeNotFound)?;
        let slot = self.slots.get_mut(index as usize).ok_or(NativeError::NodeNotFound)?;
        if slot.generation != generation || slot.node.is_none() { return Err(NativeError::NodeNotFound); }
        slot.node = None;
        self.free.push(index);
        Ok(())
    }
...
impl ContextRegistry {
    fn insert(&mut self, context: Context) -> Result<ContextHandle, NativeError> {
        ...
        if let Some(index) = self.free.pop() {
            let slot = self.slots.get_mut(index as usize).ok_or(NativeError::ContextNotFound)?;
        ...
        let index = u32::try_from(self.slots.len()).map_err(|_| NativeError::Capacity)?;
        self.slots.push(ContextSlot { generation, context: Some(context) });
    ...
    fn remove(&mut self, handle: ContextHandle) -> Result<(), NativeError> {
        ...
        let slot = self.slots.get_mut(index as usize).ok_or(NativeError::ContextNotFound)?;
        if slot.generation != generation || slot.context.is_none() { return Err(NativeError::ContextNotFound); }
        slot.context = None;
        self.free.push(index);
        Ok(())
    }
    ...
    fn get_mut(&mut self, handle: ContextHandle) -> Result<&mut Context, NativeError> {
        let (index, generation) = handle.parts().ok_or(NativeError::ContextNotFound)?;
        let slot = self.slots.get_mut(index as usize).ok_or(NativeError::ContextNotFound)?;
        if slot.generation != generation { return Err(NativeError::ContextNotFound); }
...
fn validate_context_owner(handle: ContextHandle) -> Result<(), NativeError> {
    let owners = context_owners().lock().map_err(|_| NativeError::RegistryBusy)?;
...
pub(crate) fn create_registered_context() -> Result<ContextHandle, NativeError> {
    ...
    context_owners().lock().map_err(|_| NativeError::RegistryBusy)?.insert(handle.raw(), std::thread::current().id());
    Ok(handle)
}
...
pub(crate) fn destroy_registered_context(handle: ContextHandle) -> Result<(), NativeError> {
    ...
    context_owners().lock().map_err(|_| NativeError::RegistryBusy)?.remove(&handle.raw());
    Ok(())
}
...
fn with_registry_mut<T>(
    ...
    CONTEXT_REGISTRY.with(|registry| {
        let mut registry = registry.try_borrow_mut().map_err(|_| NativeError::RegistryBusy)?;
        operation(&mut registry)
    })
}

fn next_context_generation() -> u32 { next_generation(&NEXT_CONTEXT_GENERATION) }
fn next_node_generation() -> u32 { next_generation(&NEXT_NODE_GENERATION) }
...
fn available_space(value: f32) -> AvailableSpace {
    if value.is_finite() { AvailableSpace::Definite(value.max(0.0)) } else { AvailableSpace::MaxContent }
}
...
mod tests {
    ...
    use super::{create_registered_context, destroy_registered_context, with_registered_context_mut, Context, ContextRegistry};
    ...
    fn registry_insert_and_remove_context() {
        ...
        assert!(matches!(registry.get_mut(handle), Err(NativeError::ContextNotFound)));
    }
    ...
    fn reused_context_slot_changes_generation() {
        ...
        assert!(matches!(registry.get_mut(first), Err(NativeError::ContextNotFound)));
    }

    #[test]
    fn registered_context_does_not_resolve_on_another_thread() {
        let handle = create_registered_context().unwrap();
        let other_thread = thread::spawn(move || with_registered_context_mut(handle, |_| Ok(())).unwrap_err());
    ...
    fn stale_and_cross_context_node_handles_are_rejected() {
        ...
        assert!(matches!(first_context.set_style(first, Style::default()), Err(NativeError::NodeNotFound)));
        assert!(matches!(second_context.set_style(replacement, Style::default()), Err(NativeError::NodeNotFound)));
    }
    ...
    fn flex_and_content_size_layout_execute() {
        let mut context = Context::new();
        let root = context.create_node(Style {
            display: Display::Flex,
            size: Size { width: length(100.0), height: length(50.0) },
            gap: Size { width: length(10.0), height: length(0.0) },
            ..Default::default()
        }).unwrap();
        let a = context.create_node(Style { size: Size { width: length(20.0), height: length(10.0) }, ..Default::default() }).unwrap();
        let b = context.create_node(Style { size: Size { width: length(30.0), height: length(10.0) }, ..Default::default() }).unwrap();
    ...
    fn block_flow_root_float_and_clear_execute() {
        ...
        let floated = context.create_node(Style {
            display: Display::Block,
            float: Float::Left,
            size: Size { width: length(60.0), height: length(40.0) },
            ..Default::default()
        }).unwrap();
        let cleared = context.create_node(Style {
            display: Display::Block,
            clear: Clear::Both,
    ...
    fn calc_resources_resolve_during_layout() {
        ...
        let node = context.create_node(Style { size: Size { width, height: length(10.0) }, ..Default::default() }).unwrap();
    ...
    fn cached_measurements_are_used_without_managed_callbacks() {
        ...
        context.set_measurement(node, Some(MeasurementRecord {
            min_content: Size { width: 20.0, height: 10.0 },
            max_content: Size { width: 80.0, height: 10.0 },
            preferred: Size { width: 50.0, height: 10.0 },
            ..Default::default()
        })).unwrap();
        context.compute_layout(node, f32::INFINITY, f32::INFINITY).unwrap();
    ...
    fn grid_tracks_placement_named_areas_and_diagnostics_execute() {
        ...
        let child = context.create_node(Style {
            grid_column: Line { start: line(1), end: span(2) },
            grid_row: Line { start: line(1), end: line(2) },
            ..Default::default()
        }).unwrap();
        context.set_children(root, &[child]).unwrap();
        context.compute_layout(root, 200.0, 100.0).unwrap();
        assert!(matches!(context.detailed_layout_info(root).unwrap(), DetailedLayoutInfo::Grid(_)));
    }
    ...
    fn bulk_updates_and_one_compute_per_generation() {
        ...
        context.set_styles_bulk(&[(child, Style { size: Size { width: length(10.0), height: length(10.0) }, ..Default::default() })]).unwrap();
    ...
    fn display_none_hides_descendants() {
        let mut context = Context::new();
        let root = context.create_node(Style { display: Display::None, ..Default::default() }).unwrap();
        let child = context.create_node(Style { size: Size { width: length(50.0), height: length(20.0) }, ..Default::default() }).unwrap();
        context.set_children(root, &[child]).unwrap();
        context.compute_layout(root, 100.0, 100.0).unwrap();
