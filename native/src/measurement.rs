...
pub(crate) struct MeasurementRecord {
    pub min_content: Size<f32>,
    pub max_content: Size<f32>,
    pub preferred: Size<f32>,
    pub aspect_ratio: Option<f32>,
    pub is_replaced: bool,
    pub width_samples: Vec<MeasurementSample>,
}
...
impl MeasurementRecord {
    ...
    pub(crate) fn measure(
        ...
        measured.width = known.width.unwrap_or(measured.width);
        measured.height = known.height.unwrap_or(measured.height);

        if let Some(ratio) = self.aspect_ratio {
            match (known.width, known.height) {
                (Some(width), None) if self.is_replaced => measured.height = width / ratio,
                (None, Some(height)) if self.is_replaced => measured.width = height * ratio,
                _ => {}
            }
        }
    ...
    fn measure_for_available_width(&self, available: AvailableSpace) -> Size<f32> {
        match available {
            ...
            AvailableSpace::Definite(width) => {
                if let Some(sample) = self.closest_width_sample(width) {
                    sample.size
                } else {
                    Size {
                        width: self.preferred.width.min(width.max(0.0)),
    ...
    fn closest_width_sample(&self, width: f32) -> Option<&MeasurementSample> {
        self.width_samples.iter().min_by(|left, right| {
            let left_distance = (left.available_width - width).abs();
            let right_distance = (right.available_width - width).abs();
